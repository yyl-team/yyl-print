...
const log = {
  ...
  __buildArgs({ prefix, subfix, argv }) {
    const self = this
    let iArgv = []
  ...
  lite() {
    const self = this
    const rType = Object.assign(self.source.type, {
      success: {
        name: '✓',
        color: chalk.green
      },
    ...
    Object.keys(rType).forEach((key) => {
      if (rType[key].name.length > 1) {
        rType[key].name = rType[key].name[0]
      }
    })
    self.init({
      maxSize: 1,
  ...
  log(op, argv) {
    ...
    const r = self.__buildArgs({ prefix, subfix, argv })
